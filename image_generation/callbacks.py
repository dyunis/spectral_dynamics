...
class PlWandbEvaluator(Callback):
    ...
    def evaluate(self, trainer, pl_module):
        ...
        # denoise 10 images from scratch for visualization purposes
        samples = pl_module.p_sample_loop((10, 1, 28, 28))[-1]
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        if trainer.current_epoch % self._eval_freq == 0:
            totals, samples = self.evaluate(trainer, pl_module)
            totals = {f'{self.split}/{k}': v for k, v in totals.items()}
            grid = torchvision.utils.make_grid(samples, nrow=len(samples))
            grid = grid.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
            images = wandb.Image(grid)
            totals['samples'] = images
...
class ModelPerturber(Callback):
    ...
    def perturb_(self, model):
        ...
        for n, p in named_mat_params:
            ...
            if len(p.shape) > 2:
                new_param = new_param.reshape(orig_shape)
                if 'tconv' in n:
                    new_param = new_param.transpose(0, 1)
