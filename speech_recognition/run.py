...
import callbacks
from dataset import get_dataset, TransformedLibriSpeech
...
def main(config, savedir):
    ...
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=config.batch_size, collate_fn=TransformedLibriSpeech.collate, shuffle=True, num_workers=0, pin_memory=False)
    eval_train_loader = torch.utils.data.DataLoader(eval_trainset, batch_size=config.batch_size, collate_fn=TransformedLibriSpeech.collate, shuffle=False, num_workers=0, pin_memory=False)
    val_loader = torch.utils.data.DataLoader(valset, batch_size=config.batch_size, collate_fn=TransformedLibriSpeech.collate, shuffle=False, num_workers=0, pin_memory=False)
    ...
    trainer = pl.Trainer(
        ...
        accelerator='gpu',
        devices=1,
        deterministic=False,  # ctc/lstm can't use deterministic mode
        callbacks=callback,
        logger=None,
    )
