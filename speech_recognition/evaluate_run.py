...
def main(config, run_config):
    ...
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=run_config.batch_size * 8, shuffle=False, num_workers=0, collate_fn=collate_fn)
    val_loader = torch.utils.data.DataLoader(valset, batch_size=run_config.batch_size * 8, shuffle=False, num_workers=0, collate_fn=collate_fn)
...
def evaluate_run(cp_paths, train_loader, val_loader, model, sparsity=0.5):
    ...
    for path in tqdm(cp_paths):
        ...
        for k1, k2 in pairs:
            p1 = sd[k1]
            p2 = sd[k2]
            # with bidirectional layers, only consider the correct direction for the connection
            if p1.shape[0] // p2.shape[1] == 2:
                if 'reverse' in k2:
                    p2 = p2[:, p2.shape[1]//2:]
                else:
                    p2 = p2[:, :p2.shape[1]//2]
            ...
            U_prev = _svd1[0]
            Vh_next = _svd2[-1]
            # lstm layers have 4 submatrices
            if U_prev.shape[0] != Vh_next.shape[1]:
                U_prev = U_prev.reshape(4, U_prev.shape[0] // 4, -1)
