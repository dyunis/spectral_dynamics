...
# data args
parser.add_argument('--dataset', type=str, default='wikitext103', choices=['wikitext2', 'wikitext103'], help='dataset to use')
parser.add_argument('--chunk_size', type=int, default=2048, help='chunk size for language model')
...
# optimization args
parser.add_argument('--num_epochs', type=int, default=10, help='number of epochs to loop through the data')
parser.add_argument('--batch_size', type=int, default=4, help='batch size for offline training use 1/4 of pythia batch size 1024')
parser.add_argument('--accumulate_grad_batches', type=int, default=64, help='number of batches to accumulate before optimization step')
parser.add_argument('--lr', type=float, default=6e-4, help='batch size for offline training')
parser.add_argument('--lr_schedule', type=str, default='warmup_cosine', choices=[None, 'noam', 'warmup_cosine'], help='lr scheduler')
parser.add_argument('--warmup_steps', type=int, default=200, help='number of warmup steps for optimization')
parser.add_argument('--weight_decay', type=float, default=0.1, help='weight decay for optimizer')
parser.add_argument('--gradient_clip_val', type=float, default=1.0, help='norm of grads to clip at')
parser.add_argument('--precision', type=int, default=16, choices=[16, 32], help='precision to use for training')
