...
def main(config, savedir):
    ...
    if config.mask_epoch > -1:
        from callbacks import ModelMasker
        assert config.ckpt_path is not None
        assert os.path.exists(config.ckpt_path)
        ckpt_dir = os.path.dirname(config.ckpt_path)
        masker = ModelMasker(ckpt_dir, config.mask_epoch, config.mask_sparsity, mask_type=config.mask_type, seed=config.seed)
        callback.append(masker)
