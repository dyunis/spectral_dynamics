...
def main(config, run_configs):
    # run_configs should differ by trunk seed, but have same branch seed
    ...
    assert swept_hyps == ['ckpt_path']
    # trunk_path should exist
    trunk_ckpt_paths = [rc.ckpt_path for rc in run_configs]
    for rc in run_configs:
        assert rc.ckpt_path is not None
        assert os.path.exists(rc.ckpt_path)
    assert trunk_ckpt_paths[0] != trunk_ckpt_paths[1]
    assert pl_ckpt_path_to_step(trunk_ckpt_paths[0]) == pl_ckpt_path_to_step(trunk_ckpt_paths[1])  # same split step
    trunk_config_paths = [os.path.join(os.path.dirname(cp_path), 'config.json') for cp_path in trunk_ckpt_paths]
    trunk_config_dicts = []
    trunk_seeds = []
    for trunk_config_path in trunk_config_paths:
    ...
    savedir = os.path.join(group_dir, 'lmc_cross', f'trunk-{trunk_seeds[0]}_trunk-{trunk_seeds[1]}_split-{split_step}_seed-{branch_seeds[0]}')
